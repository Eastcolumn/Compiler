...
int checkInteger(int* p_count) {
	...
	if (checkDigit(p_count)) {													// 시작이 Digit인 경우
		...
		if (checkDigit(p_count)) {																	// - 다음 Digit인 경우
			...
			printf("There is undefined token in line %d", line);		// -가 sign 혹은 Operator로 쓰이지 않는 경우는 define되지 않았으므로 error 출력
			exit(-1);
		}
	}
	...
	if ('\"' == save[(*p_count)]) {											// 처음 문자열은 " 이여야 함
		...
		while (1) {
			if (checkDigit(p_count) || '0' == save[(*p_count)] ||			// Digit,0,Letter,Blank일 경우 반복해서 받음
				...
				printf("There is undefined token in line %d", line);		// literal string을 읽는 도중 정의되지 않은 token이 들어올 경우 오류 출력
				exit(-1);
			}
		}
	}
